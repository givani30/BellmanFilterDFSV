...
class DFSVBellmanFilter(DFSVFilter):
    ...
    @jit
    def standardize_factors(self,f, h):
        """
        Standardize factors using the log-volatilities.

        Args:
            f (jnp.ndarray): Factor values.
            h (jnp.ndarray): Log-volatilities.

        Returns:
            jnp.ndarray: Standardized factors.
        """
        return f / jnp.exp(0.5*h)
    
    @jit
    def destandardize_factors(self,f_std, h):
        """
        Destandardize factors using the log-volatilities.

        Args:
            f_std (jnp.ndarray): Standardized factor values.
            h (jnp.ndarray): Log-volatilities.

        Returns:
            jnp.ndarray: Destandardized factors.
        """
        return f_std * jnp.exp(0.5*h)
    ...
    def _block_coordinate_update_impl(
        ...
        def update_factors_std(
        log_volatility: jnp.ndarray,
        factors_pred: jnp.ndarray,
        log_vols_pred: jnp.ndarray,
    ) -> jnp.ndarray:
            """
            Solve for standardized factors given log-volatility is fixed.
            
            The system solved is:
            
            (E^{-1} * (lambda_r.T @ A_inv(lambda_r)) * E + I) f_std
            = E^{-1} * (lambda_r.T @ A_inv(observation))
                + f_std_pred + E^{-1} * (I_pred[:K, K:] @ (log_volatility - log_vols_pred)),
            
            where E = diag(exp(log_volatility/2)) and E^{-1} = 1/exp(log_volatility/2).
            """
            exp_h_half = jnp.exp(log_volatility / 2.0)  # shape (K,)
            E_inv = 1.0 / exp_h_half  # elementwise reciprocal

            # Build covariance A using current log-volatility (in raw scale)
            ...
            # Compute M = lambda_r.T @ A_inv(lambda_r)
            M = lambda_r.T @ A_inv_func(lambda_r)  # shape (K, K)
            # Apply diagonal scaling: for each entry (i,j), multiply by E_inv[i]*exp_h_half[j]
            lhs_mat = (E_inv[:, None] * M * exp_h_half[None, :]) + jnp.eye(K)

            # Compute rhs: apply E_inv to the first term and to the cross term
            term1 = E_inv * (lambda_r.T @ A_inv_func(observation))  # shape (K,)
            term2 = factors_pred  # f_std_pred remains unchanged
            term3 = E_inv * (I_pred[:K, K:] @ (log_volatility - log_vols_pred))
            rhs_vec = term1 + term2 + term3

            factors_std_updated = jnp.linalg.solve(lhs_mat, rhs_vec)
            return factors_std_updated
        ...
        def update_h_bfgs(h_init: jnp.ndarray, f: jnp.ndarray) -> jnp.ndarray:
            ...
            Args:
                ...
                f (jnp.ndarray): Standardized factor values. Will be destandardized
        ...
        # Return updated state
            # Update loop
        def body_fn(_, carry: Tuple[jnp.ndarray, jnp.ndarray]) -> Tuple[jnp.ndarray, jnp.ndarray]:
            """
            Single iteration of the block-coordinate update.
            `carry` is (f_std, h).
            """
            f_current, h_current = carry
            f_new = update_factors_std(h_current, factors_std_pred, log_vols_pred)
    ...
    def predict(
        self,
        params: Union[DFSV_params],
        state: np.ndarray,
        cov: np.ndarray,
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
        Perform the Bellman prediction step for standardized state.

        This version assumes that the state is defined as
        [f_std; h],
        where f_std are the standardized factors:
        f_std = f / exp(h/2),
        and h are the log-volatilities.

        The raw factors are reconstructed as
        f = f_std * exp(h/2),
        and their prediction follows
        f_raw_pred = Phi_f @ f.
        The predicted standardized factors are then given by
        f_std_pred = f_raw_pred / exp(h_pred/2),
        where h_pred = mu + Phi_h @ (h - mu).

        The covariance update is left unchanged (although ideally it should
        also reflect the new state dynamics).
        """
        ...
        factors_std = state[:K]
        log_vols = state[K:]
        factors_raw = factors_std * jnp.exp(log_vols / 2)
        # Predict factors: E[f_{t+1}|t] = Phi_f @ f_t
        predicted_factors = Phi_f @ factors_raw
