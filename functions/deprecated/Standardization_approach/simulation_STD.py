...
class DFSV_params:
    ...
    def __post_init__(self):
        ...
        if self.lambda_r.shape != (self.N, self.K):
            raise ValueError(
                f"lambda_r should be shape ({self.N}, {self.K}), got {self.lambda_r.shape}"
            )

        if self.Phi_f.shape != (self.K, self.K):
            raise ValueError(
                f"Phi_f should be shape ({self.K}, {self.K}), got {self.Phi_f.shape}"
            )

        if self.Phi_h.shape != (self.K, self.K):
            raise ValueError(
                f"Phi_h should be shape ({self.K}, {self.K}), got {self.Phi_h.shape}"
            )

        # Allow both (K,1) or (K,) for the mean vector
        if self.mu.shape not in [(self.K, 1), (self.K,)]:
            raise ValueError(
                f"mu should be shape ({self.K}, 1) or ({self.K},), got {self.mu.shape}"
            )

        # Allow (N,N), (N,) or (N,1) for the idiosyncratic variance
        if self.sigma2.shape not in [(self.N, 1), (self.N,), (self.N, self.N)]:
            raise ValueError(
                f"sigma2 should be shape ({self.N}, 1) or ({self.N},) or ({self.N},{self.N}), got {self.sigma2.shape}"
            )

        # Check dimensions for Q_h (noise covariance matrix for log-volatilities)
        if self.Q_h.shape != (self.K, self.K):
            raise ValueError(
                f"Q_h should be shape ({self.K}, {self.K}), got {self.Q_h.shape}"
            )
...
def simulate_DFSV(
    ...
    tuple
        ...
        standardized_factors : np.ndarray
            Simulated latent standardized factors with shape (T, K)
        log_vols : np.ndarray
    ...
    for t in range(1, T):
        ...
        standardized_factors_t = factors_t / np.exp(log_vols_t / 2)
    ...
    return returns_t, standardized_factors_t, log_vols_t
