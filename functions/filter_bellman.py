...
class DFSVBellmanFilter_JAXOPT(DFSVFilter):
    """
    ...
    def _compile_jax_functions(self):
        ...
        @jax.jit
        def jax_bellman_objective(alpha, predicted_state, I_pred, observation):
            """
        ...
        @jax.jit
        def explicit_grad_bellman(alpha, predicted_state, I_pred, observation):
            """
        ...
        def obj_and_grad_fn(x, predicted_state, I_pred, observation):
            ...
            obj = jax_bellman_objective(
                x_reshaped, predicted_state, I_pred, observation
            )
            grad = explicit_grad_bellman(
                x_reshaped, predicted_state, I_pred, observation
            )
        ...
        # Compile solver
        self.solver = jaxopt.LBFGS(
            fun=self.obj_and_grad_fn,
            value_and_grad=True,
            maxiter=100,
            verbose=False,
        )
    ...
    def update(
        ...
        result = self.solver.run(
            init_params=jax_alpha_0,
            predicted_state=jax_predicted_state,
            I_pred=jax_I_pred,
...
class DFSVBellmanFilter_JAXOPT2(DFSVFilter):
    """
    Bellman Filter implementation for DFSV models using JAX for automatic differentiation. This one combines the objective and gradient in one function to simplify calculations
...
class DFSVBellmanFilter_JAXOPT3(DFSVFilter):
    """
    Bellman Filter implementation for DFSV models using JAX for automatic differentiation. This one combines the objective and gradient in one function to simplify calculations
