...
def test_boundary_values_hybrid(base_params):
    ...
    # Check Phi diagonal elements are close to tanh(safe_arctanh(original diagonals))
    expected_diag_f = jnp.tanh(safe_arctanh(jnp.diag(extreme_params.Phi_f)))
    expected_diag_h = jnp.tanh(safe_arctanh(jnp.diag(extreme_params.Phi_h)))
    np.testing.assert_allclose(
