...
if __name__ == "__main__":
    ...
    print(
        f"Precision: {'float64' if jax.config.jax_enable_x64 else 'float32'}"
    )  # Updated logic to reflect config
    ...
    returns_orig, _, _ = simulate_DFSV(params=params_orig, T=T_OBS, seed=SEED + 1)
    ...
    print(
        f"Final Log Likelihood (last run): {log_likelihood:.4f}"
    )  # Print LL as a sanity check
