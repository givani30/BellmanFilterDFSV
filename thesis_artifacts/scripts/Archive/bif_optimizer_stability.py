...
def bif_objective(
    ...
    total_log_lik = filter_instance.jit_log_likelihood_wrt_params()(
        params, returns
    )  # JIT version
    ...
    safe_neg_log_post = jnp.where(
        jnp.isnan(neg_log_post) | jnp.isinf(neg_log_post), jnp.inf, neg_log_post
    )
...
OptimizerResult = namedtuple(
    "OptimizerResult",
    ["optimizer_name", "success", "final_loss", "steps", "time_taken", "error_message"],
)
...
def run_comparison(
    ...
    for name, solver in optimizers_to_test.items():
        ...
        objective_fn_for_loss_calc = (
            bif_objective  # Use non-transformed for final loss calculation
        )
        ...
        if solver_success:
            try:
                ...
                final_loss_recalculated = objective_fn_for_loss_calc(
        ...
        success = (
            solver_success
            and (check_exception is None)
            and jnp.isfinite(final_loss_recalculated)
        )
        final_loss = (
            final_loss_recalculated if success else jnp.inf
        )  # Use recalculated loss if successful
...
def print_results_table(results: list[OptimizerResult]):
    ...
    for res in sorted(results, key=lambda x: x.optimizer_name):  # Simplified sort key
        ...
        print(
            f"{res.optimizer_name:<15} | {success_str:<8} | {loss_str:<15} | {steps_str:<8} | {time_str:<10} | {error_str}"
        )
    print("-" * 90)  # Adjusted width
