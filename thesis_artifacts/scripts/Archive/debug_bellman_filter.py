...
def run_bellman_debug(N, K, T, seed):
    ...
    try:
        ...
        if hasattr(bf, "get_filtered_factors") and hasattr(
            bf, "get_filtered_volatilities"
        ):
            ...
            metrics["bf_rmse_f"], metrics["bf_corr_f"] = calculate_accuracy(
                true_factors, filtered_factors
            )
            metrics["bf_rmse_h"], metrics["bf_corr_h"] = calculate_accuracy(
...
def plot_debug_results(metrics, true_states, filtered_states, output_dir="outputs"):
    ...
    fig, axes = plt.subplots(
        num_rows, num_cols, figsize=(12, num_rows * 3), sharex=True
    )
    ...
    for k in range(K):
        ...
        ax_f.plot(
            time_axis, filt_f[:, k], label="Filtered (BF)", color="blue", alpha=0.8
        )
        ax_f.set_ylabel(f"Factor {k + 1}")
        ax_f.legend()
        ax_f.grid(True, linestyle="--", alpha=0.6)
        rmse_f_k = (
            metrics["bf_rmse_f"][k] if metrics["bf_rmse_f"] is not None else np.nan
        )
        corr_f_k = (
            metrics["bf_corr_f"][k] if metrics["bf_corr_f"] is not None else np.nan
        )
        ...
        ax_h.plot(
            time_axis, filt_h[:, k], label="Filtered (BF)", color="red", alpha=0.8
        )
        ax_h.set_ylabel(f"Log-Vol {k + 1}")
        ax_h.legend()
        ax_h.grid(True, linestyle="--", alpha=0.6)
        rmse_h_k = (
            metrics["bf_rmse_h"][k] if metrics["bf_rmse_h"] is not None else np.nan
        )
        corr_h_k = (
            metrics["bf_corr_h"][k] if metrics["bf_corr_h"] is not None else np.nan
        )
