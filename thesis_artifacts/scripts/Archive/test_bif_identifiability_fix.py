...
def create_constrained_simple_model(N=3, K=1, seed=42):
    ...
    diag_indices = jnp.diag_indices(
        n=min(N, K), ndim=2
    )  # Correct way for (N, K) matrix
...
def constrained_transformed_objective(transformed_params, args):
    ...
    log_lik = filter_instance.jit_log_likelihood_wrt_params()(
        constrained_params, y
    )  # Pass unpacked y
...
def run_optimization(
    true_params: DFSVParamsDataclass, returns: jnp.ndarray, max_steps: int = 200
):
    """
    ...
    try:
        ...
        try:
            ...
            final_params_constrained = apply_identification_constraint(
                params_final_pos_diag
            )
            ...
            # Pass static_args directly as the 'args' tuple
            final_loss_recalculated = constrained_transformed_objective(
                final_y, static_args
            )
            ...
            print(
                f"Exception during final parameter processing/loss calculation: {validation_exception}"
            )
        ...
        if success:
            ...
            if validation_exception is not None:
                error_msg += (
                    f"; Final param/loss processing failed: {validation_exception}"
                )
        ...
        Exception
    ) as e:  # Catch exceptions during the main optx.minimise call or initial calc
        end_time = time.time()
        time_taken = end_time - start_time
        success = False
...
def print_result_summary(result: OptimizationResult):
    ...
    print(
        f"{'Optimizer':<15} | {'Success':<8} | {'Final Loss':<15} | {'Steps':<8} | {'Time (s)':<10} | {'Error Message'}"
    )
    print("-" * 80)
    success_str = "Yes" if result.success else "No"
    loss_str = (
        f"{result.final_loss:.4e}" if np.isfinite(result.final_loss) else "Inf/NaN"
    )
    ...
    print(
        f"{result.optimizer_name:<15} | {success_str:<8} | {loss_str:<15} | {steps_str:<8} | {time_str:<10} | {error_str}"
    )
    print("-" * 80)


def print_parameter_comparison(
    result: OptimizationResult, true_params: DFSVParamsDataclass
):
    ...
    if result.final_params_constrained is None:
        ...
        print(
            "  No final constrained parameters available for comparison (optimization failed or errored)."
        )
        return
    ...
    print("-" * 80)
    print(
        f"{'Parameter':<10} | {'True Value (Constrained)':<35} | {'Estimated Value (Constrained)'}"
    )
    print("-" * 80)
...
def main():
    ...
    try:
        ...
        true_objective_value = constrained_transformed_objective(
            true_params_transformed, (returns, filter_instance_for_true)
        )

        if jnp.isfinite(true_objective_value):
            print(
                f"Objective function value at TRUE parameters (Constrained): {true_objective_value:.4f}"
            )
        else:
