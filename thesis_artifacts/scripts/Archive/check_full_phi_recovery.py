...
def create_true_params(N: int, K: int, seed: int = 42) -> DFSVParamsDataclass:
    ...
    key_lambda, key_phi_f, key_phi_h, key_mu, key_sigma, key_q = jax.random.split(
        key, 6
    )

    # Factor loadings (Lambda_r): Lower triangular, diag=1
    lambda_r_init = (
        jax.random.normal(key_lambda, (N, K)) * 0.3 + 0.5
    )  # Smaller variance init
    ...
    def create_stable_matrix(key, size, max_eig=0.98, min_eig=0.8):
        ...
        try:
            ...
            if max_abs_eig >= 1.0:
                print(
                    f"Warning: Rescaling matrix, max eigenvalue was {max_abs_eig:.4f}"
                )
...
def objective_wrapper(
    ...
    args_tuple: Tuple[
        DFSVBellmanInformationFilter, jax.Array, jax.Array, bool, bool
    ],  # Static args
) -> jax.Array:
    """
    ...
    filter_instance, observations, true_mu, fix_mu_flag, phi_f_is_diagonal_flag = (
        args_tuple
    )
    ...
    if phi_f_is_diagonal_flag:
        params_constrained = dataclasses.replace(
            params_constrained, Phi_f=jnp.diag(jnp.diag(params_constrained.Phi_f))
        )
...
def run_single_replicate(
    ...
    try:
        ...
        try:
            ...
            if config["phi_f_is_diagonal"]:
                true_params_transformed = eqx.tree_at(
                    ...
                    transform_params(
                        dataclasses.replace(
                            true_params, Phi_f=jnp.diag(jnp.diag(true_params.Phi_f))
                        )
                    ).Phi_f,
                )
            ...
            if not jnp.isfinite(obj_at_true):
                print(
                    f"  WARNING: Objective at true parameters is non-finite: {obj_at_true}"
                )
            else:
        ...
        if config["phi_f_is_diagonal"]:
            initial_guess = dataclasses.replace(
                initial_guess, Phi_f=jnp.diag(jnp.diag(initial_guess.Phi_f))
            )
        ...
        scheduler = optax.warmup_cosine_decay_schedule(
            ...
            decay_steps=MAX_STEPS,  # Adjust decay steps
        )
        optimizer = optax.apply_if_finite(
            optax.adamw(learning_rate=scheduler), max_consecutive_errors=10
        )
        ...
        try:
            ...
            return ReplicateResult(
                replicate_id, False, float(jnp.inf), 0, None, time_taken, error_msg
            )
            ...
            try:
                ...
                if loss_finite:
                    ...
                    try:
                        ...
                        if config["phi_f_is_diagonal"]:
                            final_params_untransformed = dataclasses.replace(
                                final_params_untransformed,
                                Phi_f=jnp.diag(
                                    jnp.diag(final_params_untransformed.Phi_f)
                                ),
                            )
                        ...
                        print(
                            f"  ERROR untransforming final parameters: {untransform_e}"
                        )
    ...
    time_taken = time.time() - start_time
    print(
        f"  Replicate {replicate_id + 1} {'Success' if success else 'Failed'}. Loss: {float(final_loss):.4f}, Steps: {steps}, Time: {time_taken:.2f}s"
    )
    if not success:
...
def print_param_comparison(
    true_p: DFSVParamsDataclass, est_p: Optional[DFSVParamsDataclass], replicate_id: int
):
    """Prints a comparison table for true vs estimated parameters."""
    if est_p is None:
        print(
            f"  Replicate {replicate_id + 1}: Estimation failed, cannot compare parameters."
        )
        return
    ...
    # Q_h (diagonal)
    print(
        f"  Q_h (diag)| {np.asarray(jnp.diag(true_p.Q_h))} | {np.asarray(jnp.diag(est_p.Q_h))}"
    )
...
def main(args):
    ...
    print(
        f"N={args.N}, K={args.K}, T={args.T}, Replicates={args.replicates}, Seed={args.seed}"
    )
    ...
    if args.experiment == 1:
        ...
        raise ValueError(
            f"Invalid experiment number: {args.experiment}. Use 1, 11, or 2."
        )
    print(f"Configuration: {config_desc}")
    print(
        f"Stability Penalty Weight: {'N/A' if config['phi_f_is_diagonal'] else STABILITY_PENALTY_WEIGHT}"
    )
    ...
    for i in range(args.replicates):
        ...
        result = run_single_replicate(
            i, args.replicates, true_params, observations, initial_guess, config
        )
    ...
    print("\n--- Results Summary ---")
    print(
        f"{'Rep':<4} | {'Success':<8} | {'Final Loss':<15} | {'Steps':<8} | {'Time (s)':<10} | {'Error'}"
    )
    ...
    for res in all_results:
        ...
        print(
            f"{res.replicate_id + 1:<4} | {success_str:<8} | {loss_str:<15} | {steps_str:<8} | {time_str:<10} | {error_str}"
        )
    print("-" * 70)  # Adjusted width
    ...
    for res in all_results:
        if res.success:
            print_param_comparison(
                true_params, res.final_params_untransformed, res.replicate_id
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Base PRNG seed for true params and simulation keys",
    )
