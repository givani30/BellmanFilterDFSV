...
def run_single_optimizer_config_mp(
    ...
    try:
        ...
        if (
            hasattr(final_result, "result_code")
            and final_result.result_code is not None
        ):
            if final_result.result_code == optx.RESULTS.successful:
                success_str = "converged"
            elif final_result.result_code == optx.RESULTS.max_steps_reached:
                success_str = "reached max steps"
            else:
                success_str = f"code {final_result.result_code}"
        else:
            success_str = "success=True" if final_result.success else "success=False"

        print(
            f"--- Finished MP: Optimizer={optimizer_name} ({success_str}) | Loss={final_result.final_loss:.4e} | Steps={final_result.steps} | Time={final_result.time_taken:.2f}s ---"
        )
        return final_result
        ...
        end_time = time.time()
        print(
            f"--- Error MP: Optimizer={optimizer_name} failed in process {os.getpid()} after {end_time - start_time:.2f}s: {e} ---"
        )
...
def print_results_table(results: List[OptimizerResult], max_steps: int):
    ...
    print(
        f"{'Optimizer':<20} | {'Transform':<10} | {'Fix_mu':<7} | {'Status':<20} | {'Final Loss':<15} | {'Steps':<8} | {'Time (s)':<10} | {'Error Message'}"
    )
    print("-" * 160)
    for res in sorted(
    ...
    for res in sorted(
        ...
        if res.final_params is not None and res.success:  # Only show successful runs
            ...
            for param_name, true_val in true_values.items():
                est_val = est_values.get(
                    param_name, np.array([])
                )  # Handle potential missing keys if dummy params used
...
def save_parameter_errors_to_csv(
    ...
    with open(csv_path, "w", newline="") as csvfile:
        ...
        header.extend(
            [f"{p}_RMSE" for p in true_values] + [f"{p}_MAE" for p in true_values]
        )
        ...
        for res in results:
            if res.final_params is not None and res.success:
                ...
                for param_name, true_val in true_values.items():
                    est_val = getattr(
                        res.final_params, param_name, jnp.empty(0)
                    ).flatten()
