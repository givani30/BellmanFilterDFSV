...
def create_simple_model(N=3, K=2):  # Default to K=2 for this experiment
    ...
    lambda_r_init = (
        np.random.randn(N, K) * 0.5 + 0.5
    )  # Generate initial random matrix (N x 2)
    ...
    lambda_r = lambda_r.at[diag_indices].set(
        1.0
    )  # Sets lambda_r[0,0] and lambda_r[1,1] to 1.0
...
def run_comparison(
    ...
    print(
        f"Using reduced learning rate schedule: init={initial_lr}, peak={peak_lr}, end={end_lr}"
    )
    ...
    optimizers_to_test = {
        ...
        "AdamW": optx.OptaxMinimiser(
            optax.apply_if_finite(optax.adamw(learning_rate=scheduler), 10),
            rtol=rtol,
            atol=atol,
            norm=optx.rms_norm,
    ...
    for name, solver in optimizers_to_test.items():
        ...
        def fixed_mu_objective_wrapper(t_params, args_tuple):
            obs, filt, fixed_mu_val, penalty_weight = (
                args_tuple  # Unpack static args including penalty weight
            )
        ...
        try:
            ...
            try:
                ...
                final_params_fixed_mu = eqx.tree_at(
                    lambda p: p.mu, final_params_untransformed_temp, true_mu
                )
                ...
                final_params_untransformed = apply_identification_constraint(
                    final_params_fixed_mu
                )  # Final params with fixed mu and constrained lambda
                ...
                final_loss_recalculated = bellman_objective(
                    final_params_untransformed, returns, filter_instance, priors=None
                )  # Pass priors=None
                if not jnp.isfinite(final_loss_recalculated):
                    # Raise an exception if the calculated loss is not finite
                    raise ValueError(
                        f"Recalculated loss is non-finite: {final_loss_recalculated}"
                    )
            except Exception as val_e:
                validation_exception = val_e
                final_loss_recalculated = (
                    jnp.inf
                )  # Ensure loss is Inf if calculation failed
                print(
                    f"Exception during final loss calculation: {validation_exception}"
                )
            ...
            if success:
                error_msg = "N/A"
                print(
                    f"Success! Final Loss: {final_loss:.4f}, Steps: {num_steps}, Time: {time_taken:.2f}s"
                )
            else:
                ...
                print(
                    f"Failed! Status: {error_msg}, Final Loss: {final_loss:.4f}, Steps: {num_steps}, Time: {time_taken:.2f}s"
                )  # Print final_loss (potentially Inf)
        ...
        if final_params_untransformed is not None:
            try:
                phi_f_eigvals = jnp.abs(
                    jnp.linalg.eigvals(final_params_untransformed.Phi_f)
                )
                phi_h_eigvals = jnp.abs(
                    jnp.linalg.eigvals(final_params_untransformed.Phi_h)
                )
                ...
                if jnp.all(phi_f_eigvals < 1.0) and jnp.all(phi_h_eigvals < 1.0):
                    ...
                    print(
                        "Stability Check: FAILED (One or more eigenvalue magnitudes >= 1.0)"
                    )
                    ...
                    if success:  # If solver thought it succeeded but stability failed
                        ...
                        success = (
                            False  # Mark as overall failure if stability check fails
                        )
...
def print_results_table(results: list[OptimizerResult]):
    ...
    # Header
    print(
        f"{'Config Name':<35} | {'Optimizer':<15} | {'Transform':<10} | {'Success':<8} | {'Final Loss':<15} | {'Steps':<8} | {'Time (s)':<10} | {'Error Message'}"
    )  # Updated header
    print("-" * 130)  # Adjusted width
    # Rows - Sort by config, then optimizer, then transform
    for res in sorted(
        results,
        key=lambda x: (x.prior_config_name, x.optimizer_name, x.uses_transformations),
    ):
        ...
        print(
            f"{res.prior_config_name:<35} | {res.optimizer_name:<15} | {'Yes' if res.uses_transformations else 'No':<10} | {success_str:<8} | {loss_str:<15} | {steps_str:<8} | {time_str:<10} | {error_str}"
...
def save_results_to_csv(
    ...
    filename: str = "bif_strong_mu_prior_fixed_lambda_results.csv",
...
def print_parameter_comparison(
    ...
    for res in sorted(
        ...
        if res.final_params is not None:
            print(
                f"\n-- Run: Config='{res.prior_config_name}' | Optimizer='{res.optimizer_name}' | Success='{'Yes' if res.success else 'No'}' --"
            )  # Updated print
            print("-" * 80)
            ...
            print(
                f"\n-- Run: Config='{res.prior_config_name}' | Optimizer='{res.optimizer_name}' --"
            )  # Updated print
            print(
...
def main():
    ...
    print(
        f"Starting BIF Fixed Mu + Constrained Lambda_f (K=2) Evaluation..."
    )  # Updated print
    ...
    true_params = DFSVBellmanInformationFilter(
        true_params.N, true_params.K
    )._process_params(true_params)
    ...
    try:
        filter_instance_for_true = DFSVBellmanInformationFilter(
            true_params.N, true_params.K
        )
        ...
        true_objective_value = temp_objective_wrapper_fixed_mu(
            ...
            (
                returns,
                filter_instance_for_true,
                true_mu,
                stability_penalty_weight_for_true,
            ),  # Pass penalty weight
        )

        if jnp.isfinite(true_objective_value):
            print(
                f"Objective function value at TRUE parameters (Fixed Mu, Constrained Lambda_f): {true_objective_value:.4f}"
            )  # Updated print
        else:
            print(
                "Objective function value at TRUE parameters (Fixed Mu, Constrained Lambda_f) is non-finite."
            )  # Updated print
    ...
    for res in results:
        if res.final_params is not None:
            ...
            config_name_safe = (
                res.prior_config_name.replace(" ", "_")
                .replace("(", "")
                .replace(")", "")
                .replace("+", "_plus_")
                .replace(",", "")
            ...
            try:
                ...
                print(
                    f"  ERROR saving parameters for {opt_name}/{config_name_safe} to {filename}: {e}"
                )
        else:
            opt_name = res.optimizer_name.replace(" ", "_")
            config_name_safe = (
            ...
            print(
                f"  Skipping parameter saving for {opt_name}/{config_name_safe} (final_params is None)"
            )
    ...
    print(
        f"\nBIF Full Phi Hybrid Integration Test (K={true_params.K}) finished."
    )  # Updated print
