...
def create_dummy_data(
    ...
    def scan_body(carry, key_t):
        ...
        h_next = (
            mu
            + Phi_h @ (h_prev - mu)
            + jax.random.multivariate_normal(key_h, jnp.zeros(K), Q_h)
        )
        f_next = Phi_f @ f_prev + jax.random.multivariate_normal(
            key_f, jnp.zeros(K), jnp.diag(jnp.exp(h_next / 2))
        )
        obs_noise = jax.random.multivariate_normal(
            key_obs, jnp.zeros(N), jnp.diag(sigma2)
        )
...
def time_function(func, *args, repetitions=3):
    ...
    for _ in range(repetitions):
        ...
        try:
            ...
            print(
                f"  ERROR during timed run of {getattr(func, '__name__', repr(func))}: {e}"
            )
            return -1.0  # Indicate error
        ...
        for K in K_vals:
            ...
            try:
                ...
                try:
                    ...
                    alpha_updated = (
                        alpha_updated_uncompiled.block_until_ready()
                    )  # Ensure computation finishes
                except Exception as e_bcu_run:
                    print(
                        f"  WARN: Failed to run BCU for alpha_updated: {e_bcu_run}. Using alpha_init."
                    )
                ...
                def bcu_impl_jit_wrapper(
                    ...
                    return filt._block_coordinate_update_impl(
                        lambda_r,
                        sigma2,
                        alpha,
                        pred_state,
                ...
                avg_time_bcu = time_function(
                    bcu_impl_jit_wrapper,
                    params.lambda_r,
                    params.sigma2,
                    alpha_init,
                    pred_state_jax.flatten(),
                    ...
                    obs_t,
                    10,
                    filt.h_solver,  # Pass static args here
                    repetitions=repetitions,
                )
                results.append(
                    {
                        "N": N,
                        "K": K,
                        "Function": "block_coordinate_update_impl_jit",
                        "Avg Time (s)": avg_time_bcu,
                    }
                )
                ...
                avg_time_fim = time_function(
                    fisher_information_jit_timed,
                    params.lambda_r,
                    params.sigma2,
                    alpha_updated,
                    repetitions=repetitions,
                ...
                results.append(
                    {
                        "N": N,
                        "K": K,
                        "Function": "fisher_information_jit",
                        "Avg Time (s)": avg_time_fim,
                    }
                )
                ...
                avg_time_logpost = time_function(
                    log_posterior_jit_timed,
                    params.lambda_r,
                    params.sigma2,
                    alpha_updated,
                    obs_t,
                ...
                results.append(
                    {
                        "N": N,
                        "K": K,
                        "Function": "log_posterior_jit",
                        "Avg Time (s)": avg_time_logpost,
                    }
                )
                ...
                avg_time_buildcov = time_function(
                    build_covariance_jit_timed,
                    params.lambda_r,
                    exp_h_dummy,
                    params.sigma2,
                    repetitions=repetitions,
                ...
                results.append(
                    {
                        "N": N,
                        "K": K,
                        "Function": "build_covariance_jit",
                        "Avg Time (s)": avg_time_buildcov,
                    }
                )
                ...
                results.append(
                    {"N": N, "K": K, "Function": "ERROR", "Avg Time (s)": -1}
                )
                # Add placeholders for other functions for this N,K if needed for table structure
                func_names = [
                    "predict_jax",
                    "update_jax",
                    "block_coordinate_update_impl_jit",
                    "fisher_information_jit",
                    "log_posterior_jit",
                    "build_covariance_jit",
                ]
                existing_funcs = {
                    r["Function"] for r in results if r["N"] == N and r["K"] == K
                }
                for fname in func_names:
                    if fname not in existing_funcs:
                        results.append(
                            {"N": N, "K": K, "Function": fname, "Avg Time (s)": -1}
                        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Profile DFSV Bellman Filter Components."
    )
    parser.add_argument(
        "--N",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Number of time steps for dummy data generation (only affects data gen)",
    )
    parser.add_argument(
        "--reps",
        type=int,
        default=5,
        help="Number of repetitions for timing each function",
    )
    parser.add_argument(
    ...
    try:
        ...
        if profiling_results:
            ...
            df_pivot = df.pivot(
                index=["N", "K"], columns="Function", values="Avg Time (s)"
            )
        ...
        for res in profiling_results:
            print(
                f"{res.get('N', '?')}\t{res.get('K', '?')}\t{res.get('Function', 'N/A')}\t{res.get('Avg Time (s)', -1):.6f}"
            )
