...
def run_comparison(
    ...
    uninformed_params_constrained = filter_instance._process_params(
        uninformed_params_constrained
    )  # Ensure JAX arrays
    ...
    for name, solver in optimizers_to_test.items():
        ...
        def fixed_mu_objective_wrapper(t_params, args_tuple):
            obs, filt, fixed_mu_val = (
                args_tuple  # Unpack static args (only fixed_mu_val)
            )
            ...
            loss = bellman_objective(
                params_fixed_constrained, obs, filt, priors=None
            )  # Pass priors=None
            return loss
...
def main():
    ...
    print(
        f"\nBIF Fixed Mu + Constrained Lambda_f (K={true_params.K}) Evaluation finished."
    )  # Updated print
