...
def objective_fit_only(mu_vector, static_args):
    ...
    _, fit_sum, _ = filter_instance.jit_log_likelihood_wrt_params()(
        current_params, y, return_components=True
    )
...
def objective_penalty_only(mu_vector, static_args):
    ...
    # Calculate likelihood components
    # Returns (total_lik, fit_sum, penalty_sum)
    _, _, penalty_sum = filter_instance.jit_log_likelihood_wrt_params()(
        current_params, y, return_components=True
    )

    # Return penalty sum (objective is minimizing negative log-likelihood = -fit + penalty)
    safe_penalty_sum = jnp.nan_to_num(
        penalty_sum, nan=1e10, posinf=1e10, neginf=-1e10
    )  # Note: neginf maps to -1e10
...
def main():
    ...
    for name, mu_vec in mu_points_to_test.items():
        ...
        try:
            ...
            mu_str = np.array2string(
                np.asarray(mu_vec), precision=4, suppress_small=True
            )
            grad_total_str = np.array2string(
                grad_total, precision=4, suppress_small=True
            )
            grad_fit_str = np.array2string(grad_fit, precision=4, suppress_small=True)
            grad_penalty_str = np.array2string(
                grad_penalty, precision=4, suppress_small=True
            )

            print(
                f"{name:<25} | {mu_str:<20} | {grad_total_str:<20} | {grad_fit_str:<20} | {grad_penalty_str:<20}"
            )

        except Exception as e:
            print(
                f"{name:<25} | {'ERROR':<20} | {'N/A':<20} | {'N/A':<20} | {'N/A':<20} | Error: {e}"
            )
            results[name] = {
            ...
            end_time = time.time()
            print(
                f"{'':<25} | {'':<20} | {'':<20} | {'':<20} | {'':<20} | (Took {end_time - start_time:.2f}s)"
            )
    ...
    print(
        f"{'Mu Point Description':<25} | {'Total Grad':<20} | {'Fit Grad + Penalty Grad':<25} | {'Difference':<20} | {'Match?':<10}"
    )
    print("-" * 105)
    all_match = True
    for name, res in results.items():
        ...
        if (
            np.any(np.isnan(grad_total))
            or np.any(np.isnan(grad_fit))
            or np.any(np.isnan(grad_penalty))
        ):
        ...
        is_close = np.allclose(
            grad_total, sum_components, atol=1e-5, rtol=1e-4
        ...
        print(
            f"{name:<25} | {grad_total_str:<20} | {sum_comp_str:<25} | {diff_str:<20} | {match_str}"
        )

    print("-" * 105)
    if all_match:
        print(
            "✅ Sanity check passed: Total gradient approximately equals the sum of fit and penalty gradients for all points."
        )
    else:
        print(
            "❌ Sanity check failed: Discrepancies found between total gradient and sum of components."
        )
