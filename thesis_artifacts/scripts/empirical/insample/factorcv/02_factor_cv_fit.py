...
INPUT_FILE = os.path.join(
    SCRIPT_DIR.parent.parent.parent, "empirical", "vw_returns_final_with_date.csv"
)
...
try:
    ...
    if df_returns.shape[1] != 95:
        raise ValueError(
            f"Input data must have N=95 columns, found {df_returns.shape[1]}"
        )
    ...
    df_returns = pd.DataFrame(
        dummy_data, index=dates, columns=[f"Asset_{i + 1}" for i in range(N)]
    )
...
print(
    f"Initialized statsmodels DynamicFactor: N={dfm_model.nobs}, K={dfm_model.k_factors}"
)
...
try:
    ...
    factor_cv_results = dfm_model.fit(
        maxiter=10000, disp=5
    )  # disp=True shows optimizer output
    ...
    if factor_cv_results is not None and hasattr(factor_cv_results, "llf"):
        if np.isfinite(factor_cv_results.llf):
            ...
            convergence_message = (
                "Model estimation completed but log-likelihood is not finite"
            )
...
if convergence_success and factor_cv_results is not None:
    ...
    try:
        ...
        sigma_eps_hat_cv_diag = np.diag(
            ssm.obs_cov[:, :, 0]
        )  # Observation error variances (N,)
        phi_f_hat_cv = ssm.transition[
            :k_factors, :k_factors, 0
        ]  # Factor VAR coefficients (K, K)
        ...
        if hasattr(factors_bunch, "smoothed") and factors_bunch.smoothed is not None:
            ...
            filtered_factors_cv = filtered_states[
                :k_factors, :
            ].T  # Transpose to get (T, K)

        print(f"Factors shape: {filtered_factors_cv.shape}")

        # Get factor covariances
        filtered_state_covs = (
            factor_cv_results.filtered_state_cov
        )  # (state_dim, state_dim, T)
        print(f"Filtered state covariances shape: {filtered_state_covs.shape}")
        filtered_state_covs_cv = filtered_state_covs[
            :k_factors, :k_factors, :
        ].transpose(2, 0, 1)  # (T, K, K)
...
if (
    convergence_success
    and lambda_hat_cv is not None
    and sigma_eps_hat_cv_diag is not None
    and filtered_factors_cv is not None
    and filtered_state_covs_cv is not None
):
    ...
    for t in range(burn_in, T):
        ...
        try:
            ...
            z_t = np.linalg.solve(
                L_t, e_t
            )  # solve_triangular might be marginally faster
            standardized_residuals_cv[t, :] = z_t
        except np.linalg.LinAlgError:
            warnings.warn(
                f"Cholesky decomposition failed for Sigma_t|t at t={t}. Skipping residual calculation."
            )
            # Leave as NaN
    ...
    standardized_residuals_cv = pd.DataFrame(
        standardized_residuals_cv, index=df_returns.index, columns=df_returns.columns
    )
    ...
    standardized_residuals_cv_post_burn = standardized_residuals_cv.iloc[
        analysis_burn_in:
    ]
    print(
        f"Standardized residuals DataFrame shape (full): {standardized_residuals_cv.shape}"
    )
    print(
        f"Standardized residuals DataFrame shape (post-burn): {standardized_residuals_cv_post_burn.shape}"
    )
...
if convergence_success and conditional_covariance_H_cv is not None:
    try:
        ...
        metadata = {
            ...
            if num_params_cv is not None and np.isfinite(num_params_cv)
            else None,  # Use num_params_cv
            ...
            "log_likelihood": float(loglik_cv_base)
            if np.isfinite(loglik_cv_base)
            else None,
            "aic": float(aic_cv) if np.isfinite(aic_cv) else None,
            "bic": float(bic_cv) if np.isfinite(bic_cv) else None,
            "phi_f_max_eigenvalue": float(final_phi_f_max_eig)
        ...
        if standardized_residuals_cv_post_burn is not None:
            ...
            eps_csv_path = os.path.join(
                factorcv_output_dir, "standardized_residuals.csv"
            )
    ...
    print(
        "Skipping DCC-compatible output generation due to estimation error or non-convergence"
    )
