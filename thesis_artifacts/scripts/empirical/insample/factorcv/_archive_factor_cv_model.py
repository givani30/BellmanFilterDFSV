...
)  # If stationary initialization were needed
import warnings
...
class FactorCVModel(MLEModel):
    ...
    def _initialize_parameter_indices(self, n_obs, k_factors):
        ...
        n_lambda_free = (
            k_factors * (k_factors - 1) // 2 + (n_obs - k_factors) * k_factors
        )
    ...
    def start_params(self):
        ...
        phi_f_guess += (
            0.01
            * np.random.randn(self.k_factors, self.k_factors)
            * (1 - np.eye(self.k_factors))
        )
        ...
        params[self.param_indices_sigma_nu] = np.log(
            np.maximum(sigma_nu_guess, self.MIN_VARIANCE)
        )
    ...
    def param_names(self):
        ...
        names += [
            f"lambda_free.{i}"
            for i in range(
                self.param_indices_lambda.stop - self.param_indices_lambda.start
            )
        ]
    ...
    def transform_params(self, unconstrained):
        ...
        constrained[self.param_indices_sigma_eps] = np.maximum(
            np.exp(unconstrained[self.param_indices_sigma_eps]), self.MIN_VARIANCE
        )
        ...
        constrained[self.param_indices_sigma_nu] = np.exp(
            unconstrained[self.param_indices_sigma_nu]
        )
        return constrained
    ...
    def untransform_params(self, constrained):
        ...
        unconstrained[self.param_indices_sigma_eps] = np.log(
            np.maximum(constrained[self.param_indices_sigma_eps], self.MIN_VARIANCE)
        )
        ...
        unconstrained[self.param_indices_sigma_nu] = np.log(
            np.maximum(constrained[self.param_indices_sigma_nu], 1e-10)
        )
    ...
    def update(self, params, **kwargs):
        ...
        for i in range(self.k_factors):
            ...
            if n_elements_in_row > 0:
                row_elements = lambda_free_params[
                    current_idx : current_idx + n_elements_in_row
                ]
        ...
        if n_elements_remaining > 0:
            ...
            if len(remaining_rows_flat) != n_elements_remaining:
                raise ValueError(
                    f"Lambda parameter size mismatch. Expected {n_elements_remaining} for remaining rows, got {len(remaining_rows_flat)}"
                )

            # Reshape with explicit dtype to ensure consistency
            reshaped_rows = remaining_rows_flat.reshape(
        ...
        phi_f_mat = params[self.param_indices_phi_f].reshape(
            self.k_factors, self.k_factors
        )
    ...
    def _calculate_stationarity_penalty(self, phi_f_mat):
        ...
        try:
            ...
            warnings.warn(
                "Eigenvalue computation failed for Phi_f. Applying large penalty."
            )
            return (
                self.STABILITY_PENALTY_WEIGHT * self.k_factors * 10
            )  # Arbitrarily large penalty
    ...
    def loglike(self, params, *args, **kwargs):
        ...
        if debug:
            ...
            print(
                f"log_sigma_eps params: {params[self.param_indices_sigma_eps][:5]}..."
            )
        ...
        try:
            ...
            if debug:
                ...
                print(
                    f"Constrained params range: [{np.min(constrained_params):.6f}, {np.max(constrained_params):.6f}]"
                )
                print(f"Constrained params mean: {np.mean(constrained_params):.6f}")
                print(
                    f"Constrained params has NaN: {np.isnan(constrained_params).any()}"
                )
                print(
                    f"Constrained params has Inf: {np.isinf(constrained_params).any()}"
                )

                # Print specific parameter groups
                print(
                    f"sigma_eps params: {constrained_params[self.param_indices_sigma_eps][:5]}..."
                )
                print(
                    f"lambda_free params: {constrained_params[self.param_indices_lambda][:5]}..."
                )
                print(f"phi_f params: {constrained_params[self.param_indices_phi_f]}")
                print(
                    f"sigma_nu params: {constrained_params[self.param_indices_sigma_nu]}"
                )
            ...
            if debug:
                ...
                print(
                    f"design matrix range: [{np.min(self.ssm['design']):.6f}, {np.max(self.ssm['design']):.6f}]"
                )
                print(f"obs_cov matrix shape: {self.ssm['obs_cov'].shape}")
                print(
                    f"obs_cov matrix range: [{np.min(self.ssm['obs_cov']):.6f}, {np.max(self.ssm['obs_cov']):.6f}]"
                )
                print(f"transition matrix shape: {self.ssm['transition'].shape}")
                print(
                    f"transition matrix range: [{np.min(self.ssm['transition']):.6f}, {np.max(self.ssm['transition']):.6f}]"
                )
                print(f"state_cov matrix shape: {self.ssm['state_cov'].shape}")
                print(
                    f"state_cov matrix range: [{np.min(self.ssm['state_cov']):.6f}, {np.max(self.ssm['state_cov']):.6f}]"
                )
        ...
        try:
            ...
            self.ssm["state_cov"] = self.ssm["state_cov"] + self.JITTER * np.eye(
                self.k_factors
            )
            ...
            if debug:
                ...
                print(
                    f"obs_cov matrix condition number: {np.linalg.cond(self.ssm['obs_cov']):.6e}"
                )
                print(
                    f"state_cov matrix condition number: {np.linalg.cond(self.ssm['state_cov']):.6e}"
                )
                print(
                    f"obs_cov matrix determinant: {np.linalg.det(self.ssm['obs_cov']):.6e}"
                )
                print(
                    f"state_cov matrix determinant: {np.linalg.det(self.ssm['state_cov']):.6e}"
                )
            ...
            if debug:
                ...
                print(
                    f"Base log-likelihood per observation: {base_loglike / self.nobs:.6f}"
                )
                print(
                    f"Base log-likelihood per variable: {base_loglike / (self.nobs * self.n_obs):.6f}"
                )

        except Exception as e:
            # Handle potential numerical issues in the Kalman filter itself
            warnings.warn(
                f"Error calculating base log-likelihood: {e}. Returning -inf."
            )
            import traceback
        ...
        if debug:
            ...
            try:
                ...
                print(
                    f"Phi_f is {'stationary' if np.max(np.abs(eigenvalues)) < 1.0 else 'non-stationary'}"
                )
            except Exception as e:
        ...
        if not np.isfinite(penalized_loglike):
            warnings.warn(
                f"Non-finite penalized log-likelihood ({penalized_loglike}). Base LL: {base_loglike}, Penalty: {penalty}. Returning -inf."
            )
            return -np.inf
