...
predicted_covariance_H_bif = np.full(
    (T, N, N), np.nan
)  # Store Sigma_t|t-1 (for completeness)
...
for t in range(T):
    ...
    P_f_t_filt = filtered_factor_covs_bif[
        t, :, :
    ]  # Shape (K, K) - Filtered factor covariance
    ...
    try:
        ...
        L_t_filt = np.linalg.cholesky(
            Sigma_t_filt + 1e-7 * np.eye(N)
