...
"""
Fix PF Average Correlation Script

This script fixes the average correlation calculation for the PF model.
...
PF_DATA_DIR = BASE_DIR / "outputs" / "empirical" / "insample" / "pf" / "data"

print("Fixing PF average correlation calculation...")

# Load correlation matrices
try:
    corr_matrices = np.load(PF_DATA_DIR / "correlation_matrices.npy")
    ...
    print(
        f"Fixed average correlation range: {np.min(avg_correlation_fixed)} to {np.max(avg_correlation_fixed)}"
    )
    print(f"Values > 1: {np.sum(avg_correlation_fixed > 1)}")

    # Save fixed average correlation
    avg_corr_path = PF_DATA_DIR / "average_correlation.npy"
    np.save(avg_corr_path, avg_correlation_fixed)
    print(f"Fixed average correlation saved to {avg_corr_path}")

except Exception as e:
    print(f"Error fixing PF average correlation: {e}")
