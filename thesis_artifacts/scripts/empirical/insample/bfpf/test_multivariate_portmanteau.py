...
def run_tests():
    ...
    for lag, result in wn_results.items():
        print(f"{lag}:")
        print(
            f"  Hosking: stat={result['hosking']['statistic']:.2f}, p-value={result['hosking']['p_value']:.4f}"
        )
        print(
            f"  Li-McLeod: stat={result['li_mcleod']['statistic']:.2f}, p-value={result['li_mcleod']['p_value']:.4f}"
        )
    ...
    var1_mod_results = multivariate_portmanteau_tests(
        var1_moderate, lags=lags, df_model=N**2
    )
    ...
    var1_strong_results = multivariate_portmanteau_tests(
        var1_strong, lags=lags, df_model=N**2
    )
    ...
    wn_high_results = multivariate_portmanteau_tests(
        white_noise_high, lags=[5], max_dimension=20
    )
    ...
    print(
        "- VAR(1) with Strong Autocorrelation: Should have very low p-values (reject H0)"
    )
