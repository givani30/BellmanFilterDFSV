...
filtered_states_pf, filtered_infos_pf, log_likelihood_pf = pf_filter.filter(
    pf_params, returns
)
...
predicted_covariance_H_pf = np.full(
    (T, N, N), np.nan
)  # Store Sigma_t|t-1 (for standardized residuals)
...
for t in range(T):
    ...
    P_f_t_pred = predicted_factor_covs_pf[
        t, :, :
    ...
    Sigma_t_pred = (
        lambda_hat_pf @ P_f_t_pred @ lambda_hat_pf.T + sigma_eps_hat_mat_pf
    ...
    if t < T:  # For all time points
        ...
        Sigma_t_filt = (
            lambda_hat_pf @ P_f_t_filt @ lambda_hat_pf.T + sigma_eps_hat_mat_pf
        )
...
analysis_burn_in = 0
standardized_residuals_pf_post_burn = standardized_residuals_pf_df.iloc[
    analysis_burn_in:
]
...
diagnostic_results_pf = {"ljung_box_squared": {}, "arch_lm": {}, "jarque_bera": {}}
...
print(
    f"Lambda heatmap saved to {os.path.join(figures_dir, 'lambda_heatmap_interactive.png')}"
)
# plt.show()
...
print(
    f"Phi_f heatmap saved to {os.path.join(figures_dir, 'phi_f_heatmap_interactive.png')}"
)
# plt.show()
...
print(
    f"Phi_h heatmap saved to {os.path.join(figures_dir, 'phi_h_heatmap_interactive.png')}"
)
# plt.show()
...
try:
    ...
    try:
        ...
        print(
            "Note: Tests performed on a random subset of series due to high dimensionality (N=95)."
        )
...
print("\nStoring PF results...")
print(
    "Note: Standardized residuals are calculated using PREDICTED states (f_t|t-1) and covariances (P_f,t|t-1)"
)
...
try:
    ...
    print(
        f"Standardized residuals CSV (using predicted states) saved to {eps_csv_path}"
    )
