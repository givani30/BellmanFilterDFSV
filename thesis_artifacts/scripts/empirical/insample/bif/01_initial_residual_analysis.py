...
INPUT_FILE = (
    "outputs/empirical/insample/bif_predicted_residuals/standardized_residuals.csv"
)
...
def run_tests_for_series(series, alpha, lb_lags, arch_lags):
    ...
    if len(series) < max(lb_lags + arch_lags):  # Ensure enough data points
        warnings.warn(
            f"Skipping series {series.name} due to insufficient data points ({len(series)})."
        )
        return None
    ...
    try:
        ...
        for lag in lb_lags:
            ...
            results["ljung_box_sq"]["pass"][lag] = (
                np.nan
            )  # Treat failure as non-pass for aggregation
...
if __name__ == "__main__":
    ...
    try:
        ...
        )  # Assuming first col is index
        residuals_df.index = pd.to_datetime(
            residuals_df.index
    ...
    with warnings.catch_warnings(record=True) as caught_warnings:
        ...
        for col in residuals_df.columns:
            series_result = run_tests_for_series(
                residuals_df[col], ALPHA, LB_LAGS, ARCH_LAGS
            )
    ...
    pass_rates = {
        "ljung_box_sq": {
            lag: (count / valid_series_count) * 100
            for lag, count in aggregated_results["ljung_box_sq"].items()
        },
        "arch_lm": {
            lag: (count / valid_series_count) * 100
            for lag, count in aggregated_results["arch_lm"].items()
        },
        "jarque_bera": (aggregated_results["jarque_bera"] / total_jb_tests) * 100
        if total_jb_tests > 0
        else 0,
    }
