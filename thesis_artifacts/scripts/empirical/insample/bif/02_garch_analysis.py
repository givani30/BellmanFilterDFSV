...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            ...
            print(
                "Index could not be parsed as datetime. Proceeding with original index."
            )
    ...
    for i, col in enumerate(residuals_df.columns):
        ...
        if series.empty:
            ...
            garch_results[col] = {
                ...
                "pvalues": None,
                "cov_matrix": None,
                "sum_alpha_beta": np.nan,
                "wald_p_value_sum_1": np.nan,
            }
        ...
        try:
            # Fit GARCH(1,1) model
            model = arch_model(
                series, vol="Garch", p=1, q=1, mean="Zero", dist="t", rescale=True
            )
            ...
            if res.convergence_flag == 0:
                ...
                sum_alpha_beta = params.get("alpha[1]", np.nan) + params.get(
                    "beta[1]", np.nan
                )
                wald_p_sum_zero = wald_test_sum_garch_zero(
                    params,
                    pd.DataFrame(cov_matrix, index=params.index, columns=params.index),
                )
                ...
                post_garch_test_results[col] = run_tests(
                    std_resid, ALPHA, LB_LAGS, ARCH_LAGS
                )
            else:
        ...
        for test_type, lags in post_garch_pass_rates.items():
            if isinstance(lags, dict):
                for lag, count in lags.items():
                    post_garch_pass_rates[test_type][lag] = (
                        count / valid_test_series_count
                    )
            else:
    ...
    if successful_fits > 0:
        ...
        for col, result in garch_results.items():
            if result["status"] == "converged":
                ...
                if (
                    wald_p_sum_zero is not None
                    and not np.isnan(wald_p_sum_zero)
                    and wald_p_sum_zero < ALPHA
                ):
        ...
        analysis_results["perc_wald_sum_rejects_H0"] = (
            count_wald_rejects / successful_fits
        )
        ...
        analysis_results["perc_wald_rejects_and_sum_gt_0_9"] = (
            count_wald_rejects_and_sum_gt_0_9 / successful_fits
        )
