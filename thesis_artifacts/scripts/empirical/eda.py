...
def line_looks_like_data(line_stripped, min_commas):
    ...
    if not parts:
        return False  # Handle empty lines split
    try:
        float(parts[0].strip())
...
def identify_potential_blocks(filepath, keywords_list, min_commas_hdr, min_commas_data):
    ...
    for i, line in enumerate(lines):
        ...
        contains_keywords = any(
            keyword.lower() in line_stripped.lower() for keyword in keywords_list
        )
        ...
        if i + 1 < len(lines):
            ...
            if next_line_stripped:
                next_line_looks_ok = line_looks_like_data(
                    next_line_stripped, min_commas_data
                ) or line_looks_like_header(next_line_stripped, min_commas_hdr)
        ...
        if is_header:
            ...
            for j in range(i - 1, max(-1, i - 6), -1):  # Look back from i-1 down to i-5
                if j < 0:
                    break  # Stop if we reach beginning of file
                ...
                prev_contains_keywords = any(
                    keyword.lower() in prev_line_stripped.lower()
                ...
                if prev_contains_keywords and not prev_is_data:
                    ...
                    if j not in identified_lines:
                        ...
                        if potential_blocks:
                            ...
                            if (
                                j - last_block_line <= 5
                            ):  # Heuristic: Allow some gap, but not too close
    ...
    for block in potential_blocks:
        if (
            block["line_number"] - last_line > 2
        ):  # Only keep if sufficiently spaced from the last kept block
...
blocks = identify_potential_blocks(
    file_path, keywords, min_commas_for_header, min_commas_for_data
)
...
if blocks:
    ...
    for block in blocks:
        ...
        print(
            f"  Line {block['line_number'] + 1}: {block['marker_text']} (Reason: {block['reason']})"
        )
    print(
    ...
    print(
        "Could not automatically identify distinct data blocks based on the v3 heuristics."
    )
...
block_info = {
    "VW_Returns_Monthly": (
        14,
        1200,
    ),  # Marker line 14 ("Avg VW Returns -- Monthly"), next marker line 1200 ("Avg EW Returns -- Monthly")
    "EW_Returns_Monthly": (
        1200,
        2386,
    ),  # Marker line 1200, next marker line 2386 ("Avg VW Returns -- Annual")
    "Num_Firms": (
        2590,
        3776,
    ),  # Marker line 2590 ("Num Firms..."), next marker line 3776 ("Avg Mkt Cap...")
    "Avg_Mkt_Cap": (
        3776,
        4963,
    ),  # Marker line 3776, next marker line 4963 ("VW Avg BE/ME...")
...
def load_fama_french_block(
    filepath, block_name, marker_line, next_marker_line, null_val
):
    ...
    try:
        ...
        df = df.filter(
            pl.col("Date").is_not_null() & pl.col("Date").str.contains(r"^\s*\d+\s*$")
        )
        ...
        df = df.filter(
            pl.sum_horizontal(pl.all().is_not_null()) > 1
        )  # Keep rows with Date + at least one value
        ...
        print(
            f"  Successfully loaded and processed block '{block_name}'. Shape: {df.shape}"
        )
        return df

    except pl.NoDataError:
        print(
            f"Warning: No data found for block '{block_name}'. Check markers and file structure."
        )
        return None
    except Exception as e:
...
for block_name in blocks_to_load:
    if block_name in block_info:
        ...
        try:
            ...
            df = load_fama_french_block(
                file_path, block_name, marker_line, next_marker_line, missing_value
            )
            if df is None and isinstance(
                e, UnicodeDecodeError
            ):  # Check if loading failed due to encoding
            ...
            try:
                df = load_fama_french_block(
                    file_path.replace("utf-8", "latin1"),
                    block_name,
                    marker_line,
                    next_marker_line,
                    missing_value,
                ...
                print(
                    f"Latin1 encoding also failed for block '{block_name}': {e_latin1}"
                )
...
vw_returns = vw_returns.with_columns(
    (pl.exclude("Date") / 100)
)  # Convert from percentage to decimal
ew_returns = ew_returns.with_columns(pl.all().replace(-99.99, None)).with_columns(
    (pl.exclude("Date") / 100)
)
# Demean
...
vw_returns_demeaned = vw_returns.with_columns(
    [
        ...
        pl.exclude("Date")
        - pl.exclude("Date").mean(),  # Subtract means from all other columns
    ]
)
...
nan_counts = vw_returns.select(
    [pl.col(column).is_null().sum().alias(column) for column in vw_returns.columns]
)

# Convert to a format that can be easily sorted
nan_counts_df = nan_counts.melt(value_name="NaN_Count", variable_name="Column")

# Filter to only columns with NaN values and sort by count in descending order
sorted_nan_counts = nan_counts_df.filter(pl.col("NaN_Count") > 0).sort(
    "NaN_Count", descending=True
)
...
nan_percentages_df = nan_percentages.melt(
    value_name="NaN_Percentage", variable_name="Column"
)

# Filter to only columns with NaN values and sort by percentage in descending order
sorted_nan_percentages = nan_percentages_df.filter(pl.col("NaN_Percentage") > 0).sort(
    "NaN_Percentage", descending=True
)
...
valid_portfolios = [
    col for col in representative_portfolios if col in vw_returns_demeaned_final.columns
]
    ...
    [
        ...
        *[
            ...
            .otherwise(
                ...
                .then(
                    pl.col(col).map_elements(lambda x: f"{x:.4f}", return_dtype=pl.Utf8)
                )
                .otherwise(
                    pl.col(col).map_elements(lambda x: f"{x:.3f}", return_dtype=pl.Utf8)
                )
            )
            .alias(col)
...
plot_cols = [
    col for col in key_portfolios_to_plot if col in vw_returns_demeaned_final.columns
]

if not plot_cols:
    print(
        "None of the selected key portfolios exist in the final DataFrame. Cannot generate plot."
    )
else:
        ...
        for j, col2 in enumerate(numeric_cols):
            if i != j:  # Skip self-correlation
                ...
                if me2 is not None and bm2 is not None:
                    ...
                    # Update grid
                    grid[me1, bm1] = (
                        np.nanmean([grid[me1, bm1], corr_value])
                        if not np.isnan(grid[me1, bm1])
                        else corr_value
                    )

# Create a DataFrame for the grid with proper labels
grid_df = pl.DataFrame(grid, schema=[f"BM{i + 1}" for i in range(grid_size)])
...
plt.title(
    "Average Correlation Matrix of 95 Fama-French Portfolios (10×10 Grid)", fontsize=14
)
...
scaler = StandardScaler(
    with_mean=False
)  # Don't center again since data is already demeaned
...
bars = ax1.bar(
    ...
    pca_results_pd["Proportion_Variance"][:n_components_to_show]
...
line = ax2.plot(
    pca_results_pd["Component"][:n_components_to_show],
    pca_results_pd["Cumulative_Variance"][:n_components_to_show]
    * 100,  # Convert to percentage
    color="firebrick",
    marker="o",
    markersize=6,
...
for threshold, color, style in zip(thresholds, colors, styles):
    ...
    first_component = (
        np.where(pca_results_pd["Cumulative_Variance"] * 100 >= threshold)[0][0] + 1
    )
...
# Set title
plt.title(
    "PCA: Variance Explained by Principal Components for 95 Portfolios", fontsize=14
)
...
print("\nPCA Summary:")
print(
    f"Number of components needed to explain 50% of variance: {np.where(pca_results_pd['Cumulative_Variance'] >= 0.5)[0][0] + 1}"
)
print(
    f"Number of components needed to explain 80% of variance: {np.where(pca_results_pd['Cumulative_Variance'] >= 0.8)[0][0] + 1}"
)
print(
    f"Number of components needed to explain 90% of variance: {np.where(pca_results_pd['Cumulative_Variance'] >= 0.9)[0][0] + 1}"
)
print(
    f"Number of components needed to explain 95% of variance: {np.where(pca_results_pd['Cumulative_Variance'] >= 0.95)[0][0] + 1}"
)
...
valid_portfolios_firms = [
    col for col in representative_portfolios if col in num_firms_final.columns
]

# Calculate cross-sectional average number of firms series
avg_num_firms = num_firms_final.select(
    [pl.col("Date"), pl.mean_horizontal(pl.exclude("Date")).alias("Avg_Num_Firms")]
)

# --- Process Market Cap data ---
# Ensure all selected portfolios exist in the market cap data
valid_portfolios_mktcap = [
    col for col in representative_portfolios if col in mkt_cap_final.columns
]

# Calculate cross-sectional average market cap series
avg_mkt_cap = mkt_cap_final.select(
    [pl.col("Date"), pl.mean_horizontal(pl.exclude("Date")).alias("Avg_Mkt_Cap_Series")]
)
...
table2_data = num_firms_df.join(mkt_cap_df, on="Portfolio", how="outer")

# Format the table for better readability
formatted_table2 = table2_data.with_columns(
    [
        pl.col("Portfolio").alias("Portfolio"),
        pl.col("Mean_Num_Firms")
        .map_elements(
            lambda x: f"{x:.1f}" if x is not None else "N/A", return_dtype=pl.Utf8
        )
        .alias("Mean Number of Firms"),
        pl.col("Mean_Mkt_Cap")
        .map_elements(
            lambda x: f"{x:.1f}" if x is not None else "N/A", return_dtype=pl.Utf8
        )
        .alias("Mean Market Cap (Millions)"),
    ]
)

# Reorder columns for presentation
formatted_table2 = formatted_table2.select(
...
ax1.plot(
    avg_characteristics_pd["Date"],
    avg_characteristics_pd["Avg_Num_Firms"],
    color=color1,
    label="Avg Num Firms",
)
...
ax2.plot(
    avg_characteristics_pd["Date"],
    avg_characteristics_pd["Avg_Mkt_Cap_Series"],
    color=color2,
    label="Avg Mkt Cap",
)
...
size_decile_map = {
    col: extract_size_decile(col) for col in mkt_cap_final.columns if col != "Date"
}
...
for decile in range(1, 11):
    ...
    if columns:
        ...
        decile_avg_series = decile_df.select(
            [
                pl.col("Date"),
                pl.mean_horizontal(pl.exclude("Date")).alias(
                    f"Decile_{decile}_Avg_Mkt_Cap"
                ),
            ]
        )
        # Calculate the time-series average of the row-wise averages
        decile_averages[f"ME{decile}"] = decile_avg_series.select(
            pl.col(f"Decile_{decile}_Avg_Mkt_Cap").mean()
        ).item()
...
plot_data = plot_data.sort(
    pl.col("Size_Decile").str.extract(r"ME(\d+)", 1).cast(pl.Int64)
)
