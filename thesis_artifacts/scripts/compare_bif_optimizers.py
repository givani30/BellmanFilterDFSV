...
def bif_objective(
    ...
    safe_neg_ll = jnp.where(
        jnp.isnan(neg_ll) | jnp.isinf(neg_ll), jnp.inf, neg_ll
    )  # Corrected: Use instance property
    return safe_neg_ll
...
def run_comparison(
    true_params: DFSVParamsDataclass, returns: jnp.ndarray, max_steps: int = 100
):
    """
    ...
    uninformed_params = filter_instance._process_params(
        uninformed_params
    )  # Ensure JAX arrays
        ...
        for use_transform in [False, True]:
            ...
            if use_transform:
                ...
                def transformed_objective_wrapper(t_params, args_tuple):
                    ...
                    is_params_finite = jnp.all(
                        jnp.array([jnp.all(jnp.isfinite(leaf)) for leaf in leaves_in])
                    )
                    jax.debug.print(
                        "transformed_objective_wrapper: Input t_params finite: {finite}",
                        finite=is_params_finite,
                    )
                    ...
                    jax.debug.print(
                        "transformed_objective_wrapper: Output loss: {loss}, finite: {finite}",
                        loss=loss,
                        finite=is_loss_finite,
                    )
                ...
                objective_fn_for_loss_calc = (
                    bif_objective  # Use non-transformed for final loss
                )
            else:
                ...
                def objective_wrapper(params, args_tuple):
                    ...
                    jax.debug.print(
                        "objective_wrapper: Input params finite: {finite}",
                        finite=is_params_finite,
                    )
                    # Removed conditional print: if not is_params_finite: jax.debug.print(...)
                    # --- END DEBUG ---
                    ...
                    is_loss_finite = jnp.isfinite(loss)
                    jax.debug.print(
                        "objective_wrapper: Output loss: {loss}, finite: {finite}",
                        loss=loss,
                        finite=is_loss_finite,
                    )
                    # Removed conditional print: if not is_loss_finite: jax.debug.print(...)
            ...
            try:
                ...
                is_grad_finite = jnp.all(
                    jnp.array([jnp.all(jnp.isfinite(leaf)) for leaf in grad_leaves])
                )
                ...
                if (
                    initial_error and initial_error.is_not_ok
                ):  # Prioritize checkify error message
            ...
            try:
                ...
                final_error = (
                    None  # Holds the checkify.Error object from the final check
                )
                check_exception = (
                    None  # Holds any Python exception during the final check
                )
                final_loss_recalculated = jnp.inf
                final_params_untransformed = None
                if (
                    solver_success
                ):  # Only attempt validation if solver thinks it succeeded
                    try:
                        # Get final parameters (untransformed) - this might fail if sol.value is bad
                        final_params_untransformed = (
                            untransform_params(sol.value)
                            if use_transform
                            else sol.value
                        )
                        # Checkify the objective function (use the one appropriate for loss calc)
                        checkified_objective = checkify.checkify(
                            objective_fn_for_loss_calc,
                            errors=checkify.float_checks | checkify.user_checks,
                        )
                        # Call it to get the final error state and loss
                        final_error, final_loss_recalculated = checkified_objective(
                        ...
                        final_loss_recalculated = (
                            jnp.inf
                        )  # Loss is invalid if check failed
                ...
                final_loss = (
                    final_loss_recalculated if final_error is not None else jnp.inf
                )
                ...
                if success:
                    ...
                    if not solver_success:
                        ...
                        error_msg = (
                            f"Solver failed ({sol.result}) - final check skipped"
                        )
                    print(
                        f"Failed! Status: {error_msg}, Final Loss: {final_loss}, Steps: {num_steps}, Time: {time_taken:.2f}s"
                    )

            except (
                Exception
            ) as e:  # Catch exceptions during the main optx.minimise call
                end_time = time.time()
                time_taken = end_time - start_time
                success = False
...
def print_results_table(results: list[OptimizerResult]):
    ...
    # Header
    print(
        f"{'Optimizer':<15} | {'Transform':<10} | {'Success':<8} | {'Final Loss':<15} | {'Steps':<8} | {'Time (s)':<10} | {'Error Message'}"
    )
    print("-" * 90)
    # Rows
    for res in sorted(
        results, key=lambda x: (x.optimizer_name, x.uses_transformations)
    ):
        ...
        print(
            f"{res.optimizer_name:<15} | {'Yes' if res.uses_transformations else 'No':<10} | {success_str:<8} | {loss_str:<15} | {steps_str:<8} | {time_str:<10} | {error_str}"
        )
    print("-" * 90)
