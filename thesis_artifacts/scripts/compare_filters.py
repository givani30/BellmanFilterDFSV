...
def calculate_rmse(true_values, estimated_values):
    ...
    return [
        np.sqrt(mean_squared_error(true_np[:, k], est_np[:, k]))
...
def create_visual_comparison(save_path=None):
    ...
    pf = DFSVParticleFilter(
        N=params.N, K=params.K, num_particles=8000
    )  # Instantiate with N, K
    start_time_pf = time.perf_counter()
    pf_filtered_states, pf_filtered_covs, pf_log_likelihood = pf.filter(
        params=params, observations=returns
    )  # Pass params
    ...
    # Use filter method
    bif_filtered_states, bif_filtered_infos, bif_pseudo_log_likelihood_lange = (
        bif.filter(params, returns)
    )
    ...
    bif_factor_corr = [
        np.corrcoef(true_factors[:, k], bif_filtered_factors[:, k])[0, 1]
        for k in range(params.K)
    ]
    bif_vol_corr = [
        np.corrcoef(true_log_vols[:, k], bif_filtered_log_vols[:, k])[0, 1]
        for k in range(params.K)
    ]
    ...
    print("-" * 60)
    print(
        f"(Pseudo) LogLik        | {bf_pseudo_log_likelihood:<15.2f} | {pf_log_likelihood:<15.2f} | {bif_pseudo_log_likelihood_lange:<15.2f}*"
    )
    print(
        f"Time (s)               | {bf_time:<15.2f} | {pf_time:<15.2f} | {bif_time:<15.2f}"
    )
    print(
        f"Mean Factor Corr       | {np.mean(bf_factor_corr):<15.3f} | {np.mean(pf_factor_corr):<15.3f} | {np.mean(bif_factor_corr):<15.3f}"
    )
    print(
        f"Mean LogVol Corr       | {np.mean(bf_vol_corr):<15.3f} | {np.mean(pf_vol_corr):<15.3f} | {np.mean(bif_vol_corr):<15.3f}"
    )
    print(
        f"Mean Factor RMSE       | {np.mean(bf_factor_rmse):<15.3f} | {np.mean(pf_factor_rmse):<15.3f} | {np.mean(bif_factor_rmse):<15.3f}"
    )
    print(
        f"Mean LogVol RMSE       | {np.mean(bf_vol_rmse):<15.3f} | {np.mean(pf_vol_rmse):<15.3f} | {np.mean(bif_vol_rmse):<15.3f}"
    )
    print("-" * 60)
    print("* BIF Pseudo LogLik uses Lange (2024) Eq. 40 penalty term.")
    print(
        "Factor Corrs:",
        [f"{c:.3f}" for c in bf_factor_corr],
        [f"{c:.3f}" for c in pf_factor_corr],
        [f"{c:.3f}" for c in bif_factor_corr],
    )
    print(
        "LogVol Corrs:",
        [f"{c:.3f}" for c in bf_vol_corr],
        [f"{c:.3f}" for c in pf_vol_corr],
        [f"{c:.3f}" for c in bif_vol_corr],
    )
    print(
        "Factor RMSEs:",
        [f"{r:.3f}" for r in bf_factor_rmse],
        [f"{r:.3f}" for r in pf_factor_rmse],
        [f"{r:.3f}" for r in bif_factor_rmse],
    )
    print(
        "LogVol RMSEs:",
        [f"{r:.3f}" for r in bf_vol_rmse],
        [f"{r:.3f}" for r in pf_vol_rmse],
        [f"{r:.3f}" for r in bif_vol_rmse],
    )
    print("-" * 60)
    ...
    gs = fig.add_gridspec(
        5, 2, height_ratios=[3, 3, 2, 2, 1.5]
    )  # Adjust ratios, give table more space
    ...
    ax5.plot(
        bif_filtered_factors[:, 0] - true_factors[:, 0], "g:", label="BIF Error"
    )  # Add BIF
    ...
    ax6.plot(
        bif_filtered_factors[:, 1] - true_factors[:, 1], "g:", label="BIF Error"
    )  # Add BIF
    ...
    ax7.plot(
        bif_filtered_log_vols[:, 0] - true_log_vols[:, 0], "g:", label="BIF Error"
    )  # Add BIF
    ...
    ax8.plot(
        bif_filtered_log_vols[:, 1] - true_log_vols[:, 1], "g:", label="BIF Error"
    )  # Add BIF
    ...
    table_data = [
        [
            "(Pseudo) LogLik",
            f"{bf_pseudo_log_likelihood:.2f}",
            f"{pf_log_likelihood:.2f}",
            f"{bif_pseudo_log_likelihood_lange:.2f}*",
        ],
        ["Time (s)", f"{bf_time:.2f}", f"{pf_time:.2f}", f"{bif_time:.2f}"],
        [
            "Mean Factor Corr",
            f"{np.mean(bf_factor_corr):.3f}",
            f"{np.mean(pf_factor_corr):.3f}",
            f"{np.mean(bif_factor_corr):.3f}",
        ],
        [
            "Mean LogVol Corr",
            f"{np.mean(bf_vol_corr):.3f}",
            f"{np.mean(pf_vol_corr):.3f}",
            f"{np.mean(bif_vol_corr):.3f}",
        ],
        [
            "Mean Factor RMSE",
            f"{np.mean(bf_factor_rmse):.3f}",
            f"{np.mean(pf_factor_rmse):.3f}",
            f"{np.mean(bif_factor_rmse):.3f}",
        ],
        [
            "Mean LogVol RMSE",
            f"{np.mean(bf_vol_rmse):.3f}",
            f"{np.mean(pf_vol_rmse):.3f}",
            f"{np.mean(bif_vol_rmse):.3f}",
        ],
    ]
    ...
    # Add overall title
    fig.suptitle(
        "Comparison of Bellman (BF), Particle (PF), and Bellman Information (BIF) Filters (N=3, K=2, T=500)",
        fontsize=16,
    )
    # Adjust layout to prevent overlap, especially with the table
    plt.tight_layout(
        rect=[0, 0.03, 1, 0.97]
    )  # Adjust rect to make space for suptitle and table note

    # Add note about BIF likelihood below the table
    fig.text(
        0.5,
        0.01,
        "* BIF Pseudo LogLik uses Lange (2024) Eq. 40 penalty term.",
        ha="center",
        va="bottom",
        fontsize=9,
    )
