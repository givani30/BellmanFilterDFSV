...
def main() -> None:
    ...
    try:
        ...
        scatter_figures = plot_scatter_comparison(
            df_success
        )  # Use scatter_figures as per user example
        logging.info("Saving scatter comparison plots...")
        for (
            param_name,
            figure,
        ) in scatter_figures.items():  # Use param_name, figure as per user example
            save_analysis_output(
                figure, f"scatter_{param_name}_comparison", output_dir
            )  # Use scatter_{param_name}_comparison as per user example
        ...
        df_for_scaling = df_for_scaling.with_columns([pl.col("config_T").alias("T")])

        for requested_metric, actual_column in metric_mapping.items():
            try:
                if actual_column in df_for_scaling.columns:
                    logging.info(
                        f"  Creating error scaling plot for {requested_metric}"
                    )
                    error_scaling_fig = create_error_scaling_plots(
                        df_for_scaling, metric_col=actual_column
                    )
                    save_analysis_output(
                        error_scaling_fig,
                        f"error_scaling_{requested_metric}",
                        output_dir,
                    )
                    plt.close(error_scaling_fig)  # Close figure after saving
                    logging.info(
                        f"  Error scaling plot for {requested_metric} saved successfully"
                    )
                else:
                    logging.warning(
                        f"  Column {actual_column} not found in DataFrame for {requested_metric}"
                    )
            except Exception as e:
                logging.warning(
                    f"  Failed to create error scaling plot for {requested_metric}: {str(e)}"
                )
        ...
        for name, table in summary_tables.items():
            save_analysis_output(
                table, f"{name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}", output_dir
            )
