...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Submit a Google Cloud Batch job for optimization replicates."
    )
    ...
    parser.add_argument(
        "--region",
        required=True,
        help="GCP Region for the Batch job (e.g., us-central1).",
    )
    parser.add_argument(
        "--gcs_bucket", required=True, help="GCS Bucket name (e.g., my-project-bucket)."
    )
    parser.add_argument(
        "--gcs_config_path",
        required=True,
        help="GCS path prefix within the bucket for the generated config file (e.g., batch_configs/optimization_study). A unique filename will be appended.",
    )
    parser.add_argument(
        "--gcs_output_path",
        required=True,
        help="GCS path prefix within the bucket for task outputs (e.g., batch_outputs/optimization_study_YYYYMMDD_HHMMSS). Task index/details will be appended by the target script.",
    )
    parser.add_argument(
        "--docker_image_uri",
        required=True,
        help="Full URI of the Docker image (e.g., us-central1-docker.pkg.dev/my-project/my-repo/my-image:latest).",
    )
    ...
    parser.add_argument(
        ...
        help="GCE machine type for tasks (Default: e2-standard-4).",
    )
    parser.add_argument(
        "--service_account",
        default=None,
        help="Email of the service account for tasks (Optional, defaults to Compute Engine default SA).",
    )
    parser.add_argument(
        "--network",
        default=None,
        help="VPC Network URI (Optional, defaults to project default 'global/networks/default').",
    )
    parser.add_argument(
        "--subnetwork", default=None, help="VPC Subnetwork URI (Optional)."
    )
    parser.add_argument(
        "--max_run_duration",
        default="10800",
        help="Maximum duration for each task in seconds (Default: 3600).",
    )
    parser.add_argument(
        "--job_name_prefix",
        default="optimization-study",
        help="Prefix for the Batch job name (Default: optimization-study).",
    )
    parser.add_argument(
        "--config_gen_script",
        default="scripts/generate_optimization_configs.py",
        help="Path to the config generation script (Default: scripts/generate_optimization_configs.py).",
    )
    parser.add_argument(
        "--target_script",
        default="scripts/run_optimization_replicate.py",
        help="Path to the target script run by tasks *inside the container* (Default: scripts/run_optimization_replicate.py).",
    )
    parser.add_argument(
        "--config_gen_args",
        default=None,
        help="Additional arguments to pass to the config generation script as a JSON string (Optional). Example: '{\"num_replicates\": 10}'",
    )
...
def generate_configurations(script_path, extra_args_json):
    ...
    if extra_args_json:
        try:
            ...
            logging.error(
                f"Error: Invalid JSON provided for --config_gen_args: {extra_args_json}"
            )
            sys.exit(1)
    ...
    try:
        ...
        if not os.path.exists(script_path):
            raise FileNotFoundError(
                f"Config generation script not found at {script_path}"
            )
        ...
        logging.error(
            f"Error running config generation script (Return Code: {e.returncode}):"
        )
...
def upload_config_to_gcs(project_id, bucket_name, gcs_path_prefix, config_json_string):
    ...
    gcs_blob_name = "/".join(
        part for part in [gcs_path_prefix.strip("/"), config_filename] if part
    )
    ...
    try:
        ...
        return (
            ...
            configs,
            gcs_blob_name,
        )  # Return blob name for potential cleanup
    except Exception as e:
        logging.error(
            f"Error uploading config list to GCS bucket '{bucket_name}' at '{gcs_blob_name}': {e}"
        )
        sys.exit(1)
...
def define_batch_job(args, config_gcs_uri, num_tasks):
    ...
    if args.machine_type not in MACHINE_RESOURCES:
        logging.warning(
            f"Machine type '{args.machine_type}' not in predefined resources. Using defaults or potentially failing. Add it to MACHINE_RESOURCES dict."
        )
    ...
    if args.service_account:
        allocation_policy.service_account = batch.ServiceAccount(
    ...
    group = batch.TaskGroup(
        ...
        parallelism=min(
            num_tasks, 1000
        ),  # Run up to 1000 tasks in parallel (Batch limit)
...
def cleanup_gcs_config(project_id, bucket_name, blob_name):
    ...
    try:
        logging.info(
            f"Attempting to clean up config file: gs://{bucket_name}/{blob_name}"
        )
        storage_client = storage.Client(project=project_id)
        bucket = storage_client.bucket(bucket_name)
        ...
        logging.warning(
            f"Warning: Failed to cleanup config file gs://{bucket_name}/{blob_name}: {cleanup_e}"
        )
...
def main():
    ...
    config_json_string = generate_configurations(
        args.config_gen_script, args.config_gen_args
    )
