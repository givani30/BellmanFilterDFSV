...
max_iterations = 10^3;
precision      = 4; % this determines roughly to how many decimal places the optimisers solve the ML optimisation
...
if t==1
    starting_values = [0.3   0.3   0.3   1.5 8 1/10];
else
    starting_values = ML_parameters(t-1,:);
end

%% Try at the starting point
NegLL0(t,1) = NegativeLogLikelihood3(starting_values,data_expanding_window,max_iterations);
...
try
[ML_parameters1(t,:),NegLL1(t,1),exitflag1(t,1)]=fminsearch('NegativeLogLikelihood3',starting_values,options,data_expanding_window,max_iterations);
catch
end
...
options  =  optimset(options , 'TolX'         ,10^(-precision));
try
[ML_parameters2(t,:),NegLL2(t,1),exitflag2(t,1)]=fminunc('NegativeLogLikelihood3',starting_values,options,data_expanding_window,max_iterations);
catch
end
...
ub = [ 1, 1, 1,inf,15,1];
try
[ML_parameters3(t,:),NegLL3(t,1),exitflag3(t,1)]=fmincon('NegativeLogLikelihood3',starting_values,[],[],[],[],lb,ub,[],options,data_expanding_window,max_iterations);
catch
end
...
minNegLL = min([NegLL0(t,1),NegLL1(t,1),NegLL2(t,1),NegLL3(t,1)]);
 ...
 [a1,Info1,predicted_a1,predicted_Info1,no_iterations1,delta_stored1] = Bellman_filter3(data_expanding_window,ML_parameters(t,:),max_iterations);
...
%%
figure
plot(ML_parameters(:,1:3),'linewidth',2)
legend('\gamma_1','\gamma_2','\gamma_3')

%%
figure
plot(log(ML_parameters(:,4:6)),'linewidth',2)
leg1 = legend('$\log(\tau)$','$\log(\lambda)$','$\log(\sigma)$');
