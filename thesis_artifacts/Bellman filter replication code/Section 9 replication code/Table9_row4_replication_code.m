...
%% OVERDISPERSED MODEL WITH c=0 FROM HERE
...
%starting_values = [gamma1 gamma2 gamma3 tau lambda sigma];
starting_values = [0.24 0.05 0.40 1.80 7.25 0.02];
max_iterations  = 10^3; % max number of iterations use din the Bellman filter
format short
tic
NegativeLogLikelihood2_with_c_set_to_zero(starting_values,clouddataobserved,max_iterations);
toc
...
[ML_parameters_new(1,:),NegLL_new(1,1),exitflag_new(1,1)]=fminsearch('NegativeLogLikelihood2_with_c_set_to_zero',starting_values,options,clouddataobserved,max_iterations);
...
[ML_parameters_new(2,:),NegLL_new(2,1),exitflag_new(2,1)]=fminunc('NegativeLogLikelihood2_with_c_set_to_zero',starting_values,options,clouddataobserved,max_iterations);
...
lb = [-1,-1,-1,0,0,0];
ub = [ 1, 1, 1,inf,30,1];
[ML_parameters_new(3,:),NegLL_new(3,1),exitflag_new(3,1)]=fmincon('NegativeLogLikelihood2_with_c_set_to_zero',starting_values,[],[],[],[],lb,ub,[],options,clouddataobserved,max_iterations);
...
format short
target_function = @(parameters) NegativeLogLikelihood2_with_c_set_to_zero(parameters,clouddataobserved,max_iterations);
...
tic
[a2] = Bellman_filter2_with_c_set_to_zero(clouddataobserved,ML_parameters_best,max_iterations);
toc
