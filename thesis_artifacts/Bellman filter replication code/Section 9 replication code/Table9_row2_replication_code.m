...
%% Set the starting values for the optimisation for the standard model with c set to zero (sigma=0)
clear ML_parameters NegLL exitflag
%starting_values = [gamma1 gamma2 gamma3 tau lambda];
starting_values = [0.33 0.33 0.33 1.8 7];
max_iterations  = 10^3; % max number of iterations use din the Bellman filter
tic
NegativeLogLikelihood1_with_c_set_to_zero(starting_values,clouddataobserved,max_iterations);
toc

%% Three optimisation routines to estimate the parameters for the standard model with c=0 (sigma=0)
precision=4;
format short
...
[ML_parameters(1,:),NegLL(1,1),exitflag(1,1)]=fminsearch('NegativeLogLikelihood1_with_c_set_to_zero',starting_values,options,clouddataobserved,max_iterations);
...
[ML_parameters(2,:),NegLL(2,1),exitflag(2,1)]=fminunc('NegativeLogLikelihood1_with_c_set_to_zero',starting_values,options,clouddataobserved,max_iterations);
...
options  =  optimset(options , 'TolX'         ,10^(-precision));
%options =  optimset(options , 'UseParallel',  true);
lb = [-1,-1,-1,0,0]; % when c is set to zero
ub = [ 1, 1, 1,inf,10]; % when c is set to zero
[ML_parameters(3,:),NegLL(3,1),exitflag(3,1)]=fmincon('NegativeLogLikelihood1_with_c_set_to_zero',starting_values,[],[],[],[],lb,ub,[],options,clouddataobserved,max_iterations);
...
target_function = @(parameters) NegativeLogLikelihood1_with_c_set_to_zero(parameters,clouddataobserved,max_iterations);
...
tic
[a1] = Bellman_filter1_with_c_set_to_zero(clouddataobserved,ML_parameters_best,max_iterations);
toc
