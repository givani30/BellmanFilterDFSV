function [a,Info,predicted_a,predicted_Info,no_iterations,delta_stored,smoothed_a] = Bellman_filter2(data,parameter_vector,max_iterations)
...
sigma  = max(abs(parameter_vector(7)),0.00001);%force to be positive
...
for t=1:t_final
  ...
  %% Optimisation for each time step
    ...
    if delta>precision
        intervention2=1;
        disp(['an intervention was made, as algorithm failed to converge at time=',num2str(t)]) 
    end
...
if and(intervention1==1,mod(t,10)==0)
    disp('Hierarchical BFGS version: intervention was made in the intitialisation of the optimisation')
end
if and(intervention2==1,mod(t,10)==0)
    disp('Hierarchical BFGS version: intervention was made in the prediction step')
end
