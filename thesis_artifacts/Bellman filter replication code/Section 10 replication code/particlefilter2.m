...
mu          = parameters(1); % change
c           = parameters(2);% change
phi         = parameters(3);% change
sigman      = parameters(4);% change
rho_vec     = parameters(5:end);% change
y           = y - mu; % change
rho0        = rho_vec(1);
%[mu, c, phi, sigman, rho_vec]

if sum(rho_vec.^2) >= 1 || phi >= 1 || phi <= 0 || sigman <= 0
     output_LLK = inf;
else
m           = length(rho_vec) - 1;
iRow        = m+1;
...
sigma2_h    = sigman^2 / (1 - phi^2) * (1 + 2 * term);
sigma_h     = sqrt(sigma2_h);
mu_h        = c / (1 - phi); % To be adjusted
...
for t = 1:T    
    ...
    % Predict
    mH(:,1)     = c + phi * mH(:,2) + sigman * yh_term(:) + sigman * scale_h .* state_errors(:,t);
    % Compute weights
    mu_y        = exp(mH(:,1) ./ 2) .* rho0 / scale_h .* state_errors(:,t);
    sigma_y     = exp(mH(:,1) ./ 2) .* scale_y;
    weights(:)  = pdf(y(t), mu_y, sigma_y);
