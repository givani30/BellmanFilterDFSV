...
def create_test_data(N=3, K=2, T=300):
    ...
    true_params = DFSVParamsDataclass(
        ...
        Phi_f=jnp.array([[0.45, 0.02], [0.01, 0.44]]),
...
def main():
    ...
    results = compare_one_cycle_configs(
        true_params=true_params, returns=returns, max_steps=400
    )
    ...
    convergence_df.to_csv(
        output_dir / f"one_cycle_convergence_analysis_{timestamp}.csv", index=False
    )
    print(
        f"Convergence analysis saved to {output_dir / f'one_cycle_convergence_analysis_{timestamp}.csv'}"
    )

    # Save results to CSV
    basic_df = save_results_to_csv(
        results=results, filename=f"one_cycle_results_{timestamp}.csv"
    )
