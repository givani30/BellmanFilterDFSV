#!/usr/bin/env python
"""
Interactive Filter Optimization Script for DFSV Models.
...
This version uses Optimistix's interactive solving capabilities to log parameter
...
def run_optimization(
    ...
    try:
        ...
        # For now, we'll use standard minimization and log parameters after each run
        # This is simpler than using interactive solving with different optimizer types
        sol = optx.minimise(
            fn=objective,
            solver=solver,
            y0=initial_y,
        ...
        # the model at different points along the optimization path
        if log_params and sol.result == optx.RESULTS.successful:
            ...
            for alpha in alphas:
                ...
                if use_transformations:
                    current_params = untransform_params(current_y)
                else:
                    current_params = current_y
            ...
            print(f"  Generated parameter history with {len(param_history)} points")
...
def main():
    ...
    print("Starting Interactive Filter Optimization...")
    ...
    T = 1500  # Reduced for faster testing
    ...
    # Run only one filter for testing parameter logging
    filter_types = [FilterType.BIF]  # Just use BIF for testing

    # Use only one optimizer
    optimizers = ["AdamW"]  # Just use AdamW for testing
    use_transformations_options = [True]  # Always use transformations for stability
    fix_mu_options = [True]  # Only use fixed mu for testing
