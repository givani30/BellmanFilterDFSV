...
predicted_covariance_H_pf = np.full((T, N, N), np.nan)   # Store Sigma_t|t-1 (for completeness)
...
for t in range(T):
    ...
    f_t_filt = filtered_factors_pf[t, :]        # Shape (K,) - Filtered factor
    P_f_t_filt = filtered_factor_covs_pf[t, :, :]  # Shape (K, K) - Filtered factor covariance

    # Calculate conditional observation mean and covariance Sigma_{t|t}
    mu_t_filt = lambda_hat_pf @ f_t_filt  # Shape (N,) - Filtered mean
    Sigma_t_filt = (lambda_hat_pf @ P_f_t_filt @ lambda_hat_pf.T
                   + sigma_eps_hat_mat_pf)  # Shape (N, N) - Filtered covariance
    ...
    if t < T-1:  # Only for t < T-1 since we don't have prediction for last time point
        f_t_pred = predicted_factors_pf[t, :]        # Shape (K,)
        P_f_t_pred = predicted_factor_covs_pf[t, :, :]  # Shape (K, K)

        mu_t_pred = lambda_hat_pf @ f_t_pred
    ...
    try:
        ...
        print(f"Warning: Cholesky failed for Sigma_t|t at t={t} for PF.")
...
try:
    ...
    pf_results_for_comparison = {
        ...
        "standardized_residuals": standardized_residuals_pf,  # Full series
        ...
        "conditional_covariance_H": conditional_covariance_H_pf,  # T x N x N
        "predicted_covariance_H": predicted_covariance_H_pf,  # T x N x N (for reference)
        "aic": float(aic_pf),
        "bic": float(bic_pf),
