...
def perturb_parameters(params, perturbation_scale=0.2):
    ...
    perturbed_params = params.replace(
        ...
        + perturbation_scale * jax.random.normal(keys[0], params.lambda_r.shape),
        Phi_f=params.Phi_f
        + perturbation_scale * jax.random.normal(keys[1], params.Phi_f.shape),
        Phi_h=params.Phi_h
        + perturbation_scale * jax.random.normal(keys[2], params.Phi_h.shape),
        mu=params.mu + perturbation_scale * jax.random.normal(keys[3], params.mu.shape),
        sigma2=params.sigma2
        + perturbation_scale * jax.random.normal(keys[4], params.sigma2.shape),
        Q_h=params.Q_h
        + perturbation_scale * jax.random.normal(keys[5], params.Q_h.shape),
    )
...
def run_parameter_optimization(true_params: DFSVParamsDataclass, returns: np.ndarray):
    ...
    # Return results
    results = {
        "bf": {"result": bf_result, "time": bf_time},
        "bif": {"result": bif_result, "time": bif_time},
        "pf": {"result": pf_result, "time": pf_time},
    }
...
def compare_parameters(true_params, optimization_results):
    ...
    bf_errors = {
        "lambda_r": param_error(
            np.array(true_params.lambda_r), np.array(bf_params.lambda_r)
        ),
    ...
    bif_errors = {
        "lambda_r": param_error(
            np.array(true_params.lambda_r), np.array(bif_params.lambda_r)
        ),
        ...
        "sigma2": param_error(
            np.array(true_params.sigma2), np.array(bif_params.sigma2)
        ),
    ...
    pf_errors = {
        "lambda_r": param_error(
            np.array(true_params.lambda_r), np.array(pf_params.lambda_r)
        ),
    ...
    # Return errors
    return {"bf": bf_errors, "bif": bif_errors, "pf": pf_errors}
